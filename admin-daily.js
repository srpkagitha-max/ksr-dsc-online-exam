...
['examTitle','codeCount','startTime','endTime','loginBefore','secondsPerQuestion','status','qbSubject','qbClass','qbLesson','rawBits'].forEach(id=>$(id)?.addEventListener('input',saveDraft));
...
function saveDraft(){try{localStorage.setItem(DRAFT_KEY,JSON.stringify({instituteId:$('instituteId')?.value,batchId:$('batchId')?.value,examId:$('examId')?.value,examTitle:$('examTitle')?.value,codeCount:$('codeCount')?.value,startTime:$('startTime')?.value,endTime:$('endTime')?.value,loginBefore:$('loginBefore')?.value,secondsPerQuestion:$('secondsPerQuestion')?.value,status:$('status')?.value,qbSubject:$('qbSubject')?.value,qbClass:$('qbClass')?.value,qbLesson:$('qbLesson')?.value,rawBits:$('rawBits')?.value,questions}))}catch(e){}}
function restoreDraft(notify=true){try{const d=JSON.parse(localStorage.getItem(DRAFT_KEY)||'null');if(!d){if(notify)flash('Saved draft ledu.','err');return}['examId','examTitle','codeCount','startTime','endTime','loginBefore','secondsPerQuestion','status','qbSubject','qbClass','qbLesson','rawBits'].forEach(id=>{if(d[id]!=null&&$(id))$(id).value=d[id]});if(Array.isArray(d.questions)&&d.questions.length){questions=d.questions;renderHealth()}if(notify)flash('Previous draft restored ✅','ok')}catch(e){if(notify)flash('Draft restore avvaledu.','err')}}
